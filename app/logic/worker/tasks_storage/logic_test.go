...
func TestTaskTreapInsert(t *testing.T) {
	...
	fmt.Println("Max depth:", getMaxDepth(treap.GetRoot()))
...
func TestTaskTreapSearch(t *testing.T) {
	...
	for i := 1; i < NUM_NODES; i++ {
		found := treap.SearchTask(&entities.Task{Exp_time: baseTime.Add(time.Duration(i) * time.Minute)})
		if found == nil{
	...
	notFound := treap.SearchTask(&entities.Task{})
	if notFound != nil {
